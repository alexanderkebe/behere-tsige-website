...
export default function LiturgyManager({ supabase }) {
    ...
    return (
      <div className="admin-panel">
        <div className="admin-header">
          <h2>{item.id ? 'Edit Liturgy Service' : 'New Liturgy Service'}</h2>
        ...
        <form className="admin-card" onSubmit={save}>
          <div className="admin-grid-2">
            <Field label="Day of Week">
              <select className="admin-input" value={item.day_of_week} onChange={(e) => change('day_of_week', e.target.value)}>
            ...
            <Field label="Title (EN)">
              <input className="admin-input" required value={item.title_en || ''} onChange={(e) => change('title_en', e.target.value)} placeholder="e.g. Sunday Divine Liturgy" />
            </Field>
            
            <Field label="Title (AM)">
              <input className="admin-input admin-input-am" value={item.title_am || ''} onChange={(e) => change('title_am', e.target.value)} placeholder="e.g. የእሑድ ቅዳሴ" />
            </Field>
    ...
    <div className="admin-panel">
      ...
      {loading ? (
        ...
        <div className="admin-list">
          {list.length === 0 && <p className="admin-status">No liturgy services scheduled yet.</p>}
          {list.map((item) => (
            <div className="admin-row" key={item.id}>
              <div className="admin-row-thumb" style={{ background: 'var(--gold-dark)', color: 'white', fontSize: '0.8rem', fontWeight: 'bold' }}>
